 ...
 require([ "jquery", "velocity-animate", "jquery.textfill"], function ($, Velocity) {
  $(function() {
    ...
    function unfoldStep1(strTarget, strBGColor) {
      ...
      objTarget = $(strTarget);
      objTarget.find(".headUnfold").velocity({ top: 100 + "px", left: 150 + "px" }, { duration: 1000});
      objTarget.find(".backUnfold").velocity({ top: -30 + "px", left: -20 + "px" }, { duration: 1000});
      objTarget.find(".backFeetUnfold").velocity({ top: -100 + "px"}, 1000, function(elements){
        console.log(elements);
        strStartTransform = "scale(1) ";
        strEndTransform = "scale(0.5) ";

        objTarget = $(elements[0]);

        objTarget.closest(".birdUnfold").find(".frontFeetUnfold").velocity({ transform: [strEndTransform, strStartTransform] }, 1000, function(){
      ...
      objTarget.find(".quarterUnfold").velocity({
        ...
        progress: function(elements, complete, remaining, start, tweenValue){
          if(complete>=0.5) {
            ...
            objTarget = $(elements[0]);
            objTarget.closest(".birdUnfold").find(".quarterUnfold").css("background-image","url(../assets/images/unfold/" + strBGColor + "/darken.png)");
          }
        },
        complete: function(elements) {

          objTarget = $(elements[0]);
          objTarget.closest(".birdUnfold").find(".step2").css("opacity", 0);
          objTarget.closest(".birdUnfold").find(".half").css("opacity", 1);
          objTarget.closest(".birdUnfold").find(".otherHalf").css("opacity", 1);
    ...
    function unfoldStep4(strTarget, strBGColor) {
      ...
      objTarget.find(".otherHalf").velocity({
        ...
        progress: function(elements, complete, remaining, start, tweenValue){
          if(complete>=0.5) {
            objTarget = $(elements[0]);
            objTarget.closest(".birdUnfold").find(".otherHalf").css("background-image","url(../assets/images/unfold/" + strBGColor + "/half2_rotated.png)");
          }
        },
        complete: function(){
            ...
            if(objNewTarget.data("flipped")==1) {
              ...
              objNewTarget.velocity({transform: [strEndTransform, strStartTransform]}, 1000,
                function() {
                  ...
                  strEndTransform = "translate(-50%, -50%) scale(" + intScaling*0.4 + ")";
    ...
    function openBird($objTarget) {
      ...
      if(0==0){
        ...
        if($objTarget.data("flipped")==1) {
          ...
          $objTarget.velocity({transform: [strEndTransform, strStartTransform]}, 1000,
            function() {
